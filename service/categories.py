...
def grabL2Category(eventDesc, L1Category):
    ...
    try:
        ...
        l2_contributing_words = [feature_names[idx] for idx in non_zero_indices]
        logger.log_message(message=f"Contributing words for prediction L2:{l2_contributing_words}",level="info" )
        predictions = loaded_model.predict(X_test_vect)

        L2 = predictions[0]
        return L2,l2_contributing_words
...
def grabL1Category(eventName, eventDesc):
    ...
    feature_names = loaded_vectorizer.get_feature_names_out()
    non_zero_indices = X_test_vect.nonzero()[1]
    l1_contributing_words = [feature_names[idx] for idx in non_zero_indices]
    logger.log_message(message=f"Contributing words for predictions {y_pred} {l1_contributing_words}",level="info")
    L1 = y_pred[0]
    L2,l2_contributing_words = grabL2Category(eventDesc, L1)
    cosine_results=classify_event(combined_input)
    ...
    if cosine_results.get("L2") is not None:
        if str(cosine_results["L2"][0]).strip() not in result["L2"]:
            result["L2"].append(cosine_results["L2"][0])
    
    result["L1"]=','.join(result["L1"])
    result["L2"]=','.join(result["L2"])
